        ...
        while (chunk = chunks[i]) {
            ...
            if (chunks[i].indexOf('COM Mortgages') >= 0) {
                ...
                do {
                    ...
                    // NOTE 'COM Mortgages' cases are now also appearing earlier in the pdf
                    // Ignore Case Numbers like eg. S ECI 2014 00381
                    if (key.match(/S\s+ECI\s+2014\s+\d{5}/)) {
                        // Continue with the next iteration of the outer loop
                        i++;
                        continue searching;
                    }
                    // check to see if we're at the end
                    done = key.match(/S\s+E?CI\s+20\d{2}\s+\d{5}/);
                // if not, keep going
                } while ( !done );
